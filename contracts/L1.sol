...
contract PenkMarketL1 is ReentrancyGuard, Ownable, Pausable {
    ...
    event PenkBonusUpdated(uint256 oldBasisPoints, uint256 newBasisPoints);
    ...
    function completeTransaction(string memory txid) external validTransaction(txid) {
        ...
        // Transfer tokens/ETH to owner when completing transaction
        if (keccak256(abi.encodePacked(transaction.tokenType)) == keccak256(abi.encodePacked("ETH"))) {
            // Transfer ETH to owner
            (bool success, ) = payable(owner()).call{value: transaction.amount}("");
    ...
    // Set PenkBonus percentage (only owner)
    function setPenkBonus(uint256 newBasisPoints) external onlyOwner {
        require(newBasisPoints <= 1000, "PenkBonus cannot exceed 10%"); // Max 10% bonus
        uint256 oldBasisPoints = penkBonusBasisPoints;
        penkBonusBasisPoints = newBasisPoints;
        emit PenkBonusUpdated(oldBasisPoints, newBasisPoints);
    }
